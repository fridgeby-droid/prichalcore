...
## Inspections module (v1.7)

Основной поток: `Черновик → Завершена`. Проверка проводится управляющим и не требует отдельного согласования.

Связи:

```text
InspectionTemplate
  ├── InspectionTemplateStore      # пусто = форма для всех точек
...
`InspectionTemplateField.violation_rule_json` хранит правило автоматического отклонения. Поддержаны `false`, `equals`, `not_equals`, `lt`, `gt`, `outside`. Управляющий также может вручную отметить любой пункт как нарушение.
