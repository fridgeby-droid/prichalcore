...
def compute_store_rating(db: Session, store_id: int):
    ...
    submitted = db.scalar(select(func.count(ShiftReport.id)).where(ShiftReport.store_id == store_id, ShiftReport.submitted_at >= start_dt, ShiftReport.submitted_at <= end_dt)) or 0
